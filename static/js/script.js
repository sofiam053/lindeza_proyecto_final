document.addEventListener('DOMContentLoaded', () => {
    ...
    // --- Ver detalles modal ---
    // Modal básico reutilizando el login modal
    ...
    document.querySelectorAll('.product-gallery-card .view-details').forEach(button => {
        button.addEventListener('click', async (e) => {
            const card = e.target.closest('.product-gallery-card');
            const id = card.dataset.id;
            // Puedes obtener los datos del producto del HTML o de la API
            const products = {
                "1": { name: "Labial Matte", price: 18.00, img: card.querySelector('img').src, desc: "Rojo Pasión, acabado mate y larga duración." },
                "2": { name: "Paleta de Sombras", price: 45.00, img: card.querySelector('img').src, desc: "Colores vibrantes, alta pigmentación y fácil difuminado." },
                "3": { name: "Base Fluida", price: 35.00, img: card.querySelector('img').src, desc: "Cobertura perfecta, textura ligera y acabado natural." },
                "4": { name: "Máscara Volumen", price: 22.00, img: card.querySelector('img').src, desc: "Pestañas con volumen extremo y duración todo el día." }
            };
            const prod = products[id];
            detailsContent.innerHTML = `
                <img src="${prod.img}" alt="${prod.name}" style="max-width:220px;display:block;margin:0 auto 1rem auto;border-radius:16px;box-shadow:0 2px 12px #d9669022;">
                <h2 style="text-align:center;color:#d96690;">${prod.name}</h2>
                <p style="text-align:center;font-size:1.1rem;color:#333;margin-bottom:1rem;">${prod.desc}</p>
                <div style="text-align:center;font-size:1.3rem;font-weight:700;color:#d96690;margin-bottom:1.2rem;">$${prod.price.toFixed(2)}</div>
                <button class="btn btn-primary add-to-cart-modal" data-id="${id}" style="display:block;margin:0 auto;">Agregar al carrito</button>
            `;
            detailsModal.classList.remove('hidden');
